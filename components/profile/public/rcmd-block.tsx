  ...
  return (
    <div className={`${blockStyles.container} ${blockStyles.card}`}>
      ...
      <div className="flex items-start justify-between mb-2">
        ...
        {rcmd.price_range && (
          <span className={blockStyles.metaText}>{String(rcmd.price_range)}</span>
        )}
      </div>
      ...
      {rcmd.tags && rcmd.tags.length > 0 && (
        <div className="flex flex-wrap gap-2 my-3">
          {rcmd.tags.map((tag) => (
            <span key={tag} className={blockStyles.tag}>{tag}</span>
          ))}
        </div>
      )}
      ...
      <div className="mt-auto pt-4 flex items-center justify-between">
        {rcmd.location ? (
          <span className={`${blockStyles.metaText} mt-4`}>📍 {String(rcmd.location)}</span>
        ) : <span></span>}
