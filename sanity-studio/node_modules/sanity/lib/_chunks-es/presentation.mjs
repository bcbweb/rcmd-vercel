...
import { useDocumentStore, usePerspective, isRecord, isReference, getDraftId, getVersionId, getPublishedId, defineLocaleResourceBundle, useTranslation, defineDocumentFieldAction, useWorkspace, pathToString, definePlugin, getVersionFromId, isDocumentSchemaType } from "sanity";
...
function listen(id, fields, store, perspectiveStack) {
  const query = {
    ...
    id,
    versions: perspectiveStack.map((p) => p === "drafts" ? getDraftId(id) : getVersionId(getPublishedId(id), p)).concat(getPublishedId(id))
  };
